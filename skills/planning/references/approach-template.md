...
|-----------|------------|--------|---------------------|
| [e.g., Stripe SDK] | **HIGH** | New external dep, no existing pattern | Spike (handled by anionzo:validating skill) |
| [e.g., Webhook handler] | **HIGH** | Security-critical, novel pattern | Spike (handled by anionzo:validating skill) |
...
### HIGH-Risk Summary (for anionzo:validating skill)

> The anionzo:validating skill will create spikes for these items. List them clearly.
...
> Items that couldn't be resolved in planning. The anionzo:validating skill's plan-checker will address these.
