...
>
> Source patterns: CE `/lfg` + `/slfg`, GSD phase loop with plan-checker and verifier, anionzo architecture v2.

---

## Overview

Go mode is the full anionzo pipeline from raw feature request to merged, compounded learnings. It chains every skill in sequence with exactly 4 human gates. The pipeline is designed so that each gate protects the next irreversible commitment.
...
Before invoking `anionzo:exploring`, always:

1. Run State Bootstrap from SKILL.md (check `.anionzo/`, read `critical-patterns.md`).
...
**Invoke:** Load `anionzo:exploring` skill.

**Input:** User's feature description.

**The anionzo:exploring skill will:**
...
**Invoke:** Load `anionzo:planning` skill.
...
**Invoke:** Load `anionzo:planning` again in current-phase preparation mode.
...
**Invoke:** Load `anionzo:swarming` skill.

**The anionzo:swarming skill will:**
...
**Invoke:** Load `anionzo:reviewing` skill only after the final phase swarm completes.

**The anionzo:reviewing skill will:**
...
Delete `.anionzo/HANDOFF.json` if it exists.
...
-> Load anionzo:exploring skill, focus on those specific gray areas
...
-> Load anionzo:swarming skill (fix-bead swarm only)
...
## Config Options (.anionzo/config.json)
