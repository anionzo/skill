---
name: using-anionzo
description: Bootstrap meta-skill for the anionzo agentic development ecosystem. Load first on any anionzo project. Lists all 9+2 skills with routing logic, session scout/bootstrap, small-change vs standard-feature vs high-risk mode selection, go mode (full-auto pipeline with 4 human gates), priority rules, and state resume. Invoke when starting a new session, choosing which skill to use, running the full pipeline end-to-end, or resuming after a handoff.
metadata:
  version: '2.2'
  ecosystem: anionzo
  dependencies:
...
- If `node` is missing or too old: stop immediately, tell the user Anionzo requires Node.js 18+, and ask them to install or upgrade Node before continuing.

Then run `node scripts/onboard_anionzo.mjs --repo-root <repo-root>` from this skill directory and inspect the JSON result.
...
- If onboarding is missing or stale:
  ...
  - if `status = "missing_runtime"`: stop, tell the user Anionzo requires Node.js 18+, and ask them to install or upgrade Node before continuing
  - if `requires_confirmation = true`, explain that an existing `compact_prompt` was found and Anionzo will preserve it unless the user explicitly approves replacement
  - ask before making repo changes
  - after approval, run `node scripts/onboard_anionzo.mjs --repo-root <repo-root> --apply`
...
- repo-local `.codex/hooks/anionzo_*.mjs`
- repo-local `.codex/anionzo_status.mjs`
- repo-local `.codex/anionzo_state.mjs`
- `.anionzo/onboarding.json`
- `.anionzo/state.json`

If onboarding is not complete, do not continue into the rest of the Anionzo workflow.
...
Every packaged Anionzo skill must make its dependency posture explicit. There are only three valid states:
...
When updating or adding packaged Anionzo skills, keep the docs and the live report aligned by running:

- `node plugins/anionzo/skills/using-anionzo/scripts/test_onboard_anionzo.mjs`
- `bash scripts/check-markdown-links.sh plugins/anionzo/skills/using-anionzo/SKILL.md`
...
|---|-------|----------------------|--------------|
| 1 | `anionzo:using-anionzo` | This file. Routing, go mode, red flags. | Starting any session |
| 2 | `anionzo:exploring` | Identify gray areas, lock decisions → CONTEXT.md | Feature request is vague or new; "what exactly should this do?" |
| 3 | `anionzo:planning` | Research + synthesis → `phase-plan.md`, then current-phase contract/story map + beads | Decisions are locked (CONTEXT.md exists); ready to show the full phase/story breakdown and prepare the next phase |
| 4 | `anionzo:validating` | Verify the current phase contract, story map, and bead graph before execution | The phase plan is approved and the current phase has stories and beads; need to prove this phase is actually execution-ready |
| 5 | `anionzo:swarming` | Launch+tend worker pool via Agent Mail + bv | Beads are validated; ready to execute at scale |
| 6 | `anionzo:executing` | Single worker loop: priority → reserve → implement bead → close → loop | Spawned by swarming; one agent, self-routing from the live graph |
| 7 | `anionzo:reviewing` | 5 parallel review agents (P1/P2/P3) + artifact verification + UAT | Execution complete; need quality gate before merge |
| 8 | `anionzo:compounding` | Capture learnings → history/learnings/ → critical-patterns.md | Feature shipped; extract patterns/decisions/failures for future runs |
| 9 | `anionzo:writing-anionzo-skills` | TDD-for-skills: RED-GREEN-REFACTOR + persuasion psychology | Improving or creating anionzo skills themselves |
| 10 | `anionzo:debugging` | Root-cause analysis for blocked beads and execution failures | Agent stuck, bead blocked, unexpected error |
| 11 | `anionzo:gkg` | Codebase intelligence via gkg MCP tools after readiness is green | Need deep codebase understanding before planning |
...
| `standard_feature` | Normal feature or refactor with clear value but moderate scope | Default mode for most Anionzo work |
...
|---|---|---|
| Vague/new feature ("build X") | `anionzo:exploring` | Always start here if gray areas exist |
| Research task ("investigate Y") | `anionzo:planning` | Skip exploring only if scope is fully clear |
| "Just fix this" / small change | `anionzo:planning` | Route in `small_change` mode |
| "Review my code" | `anionzo:reviewing` | Load directly |
| "What did we learn?" / "Capture learnings" | `anionzo:compounding` | Load directly |
| "Improve anionzo itself" | `anionzo:writing-anionzo-skills` | Load directly |
| Agent stuck / error | `anionzo:debugging` | Load directly |
| "Run the full pipeline" / `/go` | Go Mode (below) | Chain all skills |
| Resuming a session | Resume Logic (below) | Check HANDOFF.json first |

**When in doubt: invoke `anionzo:exploring` first.** The cost of over-exploring is low; the cost of executing a misunderstood feature is high.
...
```
0. Confirm Anionzo onboarding is current via .anionzo/onboarding.json
   → If missing or stale: return to Plugin Onboarding above

0.5. If .codex/anionzo_status.mjs exists: run `node .codex/anionzo_status.mjs --json`
...
1. Check for .anionzo/ directory in project root
   → If missing: mkdir -p .anionzo/ and create defaults below
   
2. Check .anionzo/state.json
...
If `.anionzo/HANDOFF.json` exists:

```
1. Read HANDOFF.json (and .anionzo/state.json if present)
...
Use this for the default Anionzo chain. This is the normal case for most feature work:
...
2. **Context budget always applies.** After each bead completion or major phase, if context >65% used: write `.anionzo/HANDOFF.json` and pause. Do not continue burning context.
...
This is the default way Codex and GPT models should communicate anywhere inside the Anionzo workflow unless a narrower skill requires something stricter.
...
```
.anionzo/
  onboarding.json   ← Anionzo plugin onboarding status + managed asset versions
...
.codex/
  anionzo_status.mjs  ← Read-only scout command for onboarding, state, and handoff
  anionzo_state.mjs   ← Shared state helpers used by the scout command
