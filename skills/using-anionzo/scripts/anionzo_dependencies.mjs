...
function formatPluginSkillName(skillName) {
  ...
  return skillName.includes(":") ? skillName : `anionzo:${skillName}`;
}
...
export function collectAnionzoSkillDependencies(options = {}) {
  const repoRoot = path.resolve(options.repoRoot || process.cwd());
  const skillsRoot = path.resolve(
    options.skillsRoot || path.join(repoRoot, "plugins", "anionzo", "skills"),
  );
...
export function buildAnionzoDependencyReport(options = {}) {
  const repoRoot = path.resolve(options.repoRoot || process.cwd());
  const skillsRoot = path.resolve(
    options.skillsRoot || path.join(repoRoot, "plugins", "anionzo", "skills"),
  );
  const declarations = collectAnionzoSkillDependencies({ repoRoot, skillsRoot });
