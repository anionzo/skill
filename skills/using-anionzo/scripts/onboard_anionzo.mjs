...
import {
  buildDefaultState,
  normalizeAnionzoState,
  readGkgReadiness,
  readAnionzoState,
  writeAnionzoState,
} from "./anionzo_state.mjs";
import { buildAnionzoDependencyReport } from "./anionzo_dependencies.mjs";

const SCRIPT_PATH = fileURLToPath(import.meta.url);
const USING_ANIONZO_DIR = path.dirname(path.dirname(SCRIPT_PATH));
const USING_ANIONZO_SCRIPTS_DIR = path.dirname(SCRIPT_PATH);
const PLUGIN_ROOT = path.dirname(path.dirname(USING_ANIONZO_DIR));
...
const HOOK_TEMPLATES_DIR = path.join(USING_ANIONZO_DIR, "templates");
const ONBOARDING_SCHEMA_VERSION = "1.0";
const COMPACT_PROMPT_MARKER_START = "# ANIONZO: compact_prompt start";
const COMPACT_PROMPT_MARKER_END = "# ANIONZO: compact_prompt end";
const MIN_NODE_MAJOR = 18;
const MANAGED_HOOK_FILENAMES = [
  "anionzo_session_start.mjs",
  "anionzo_pre_tool_use.mjs",
  "anionzo_stop.mjs",
];
const LEGACY_HOOK_FILENAMES = [
  "anionzo_session_start.py",
  "anionzo_pre_tool_use.py",
  "anionzo_stop.py",
];
const MANAGED_SUPPORT_FILES = {
  "anionzo_status.mjs": path.join(HOOK_TEMPLATES_DIR, "anionzo_status.mjs"),
  "anionzo_state.mjs": path.join(USING_ANIONZO_SCRIPTS_DIR, "anionzo_state.mjs"),
  "anionzo_dependencies.mjs": path.join(USING_ANIONZO_SCRIPTS_DIR, "anionzo_dependencies.mjs"),
};

function readDependencyHealth(repoRoot) {
  try {
    return buildAnionzoDependencyReport({ repoRoot });
  } catch (error) {
    return {
      checked_at: utcNow(),
      summary: {
...
function managedAgentsPresent(text) {
  return text.includes("<!-- ANIONZO:START -->") && text.includes("<!-- ANIONZO:END -->");
}
...
function renderCompactPromptBlock() {
  return [
    ...
    'compact_prompt = """',
    "MANDATORY: Anionzo context compaction recovery.",
    "",
    "STOP. Before doing anything else:",
    "1. Read AGENTS.md completely.",
    "2. If present, run node .codex/anionzo_status.mjs --json.",
    "3. If present, read .anionzo/HANDOFF.json, .anionzo/state.json, and .anionzo/STATE.md.",
    ...
    SessionStart: [
      {
        ...
        hooks: [
          {
            type: "command",
            command: buildManagedHookCommand("anionzo_session_start.mjs"),
    ...
    PreToolUse: [
      {
        ...
        hooks: [
          {
            type: "command",
            command: buildManagedHookCommand("anionzo_pre_tool_use.mjs"),
            statusMessage: "Anionzo: shell guardrails",
          },
        ],
      },
    ],
    Stop: [
      {
        hooks: [
          {
            type: "command",
            command: buildManagedHookCommand("anionzo_stop.mjs"),
            statusMessage: "Anionzo: end-of-turn check",
          },
        ],
      },
    ],
...
function isAnionzoHookEntry(entry) {
  for (const hook of entry?.hooks || []) {
    const command = hook?.command || "";
    const status = hook?.statusMessage || "";
    if (command.includes(".codex/hooks/anionzo_") || status.startsWith("Anionzo:")) {
      return true;
    }
  }
  return false;
...
function mergeHooksJson(hooksPath) {
  ...
  for (const [eventName, entries] of Object.entries(renderManagedHookEntries())) {
    ...
    const filtered = currentEntries.filter((entry) => !isAnionzoHookEntry(entry));
...
function needsStateUpdate(repoRoot) {
  const statePath = path.join(repoRoot, ".anionzo", "state.json");
  const existing = normalizeAnionzoState(readAnionzoState(repoRoot));
...
function buildRuntimeBlockedPayload(repoRoot, action) {
  ...
  return {
    ...
    message: `Anionzo requires Node.js ${MIN_NODE_MAJOR}+ before onboarding can continue. Install Node.js and rerun onboarding.`,
    details: {
      runtime,
    },
  };
...
export function checkRepo(repoRoot) {
  ...
  const onboardingPath = path.join(repoRoot, ".anionzo", "onboarding.json");
  ...
  if (!agentsExists) {
    ...
    actions.push("append_anionzo_managed_block_to_AGENTS.md");
  }
  ...
  if (!configText) {
    ...
    if (compactPromptConflict) {
      ...
      actions.push("install_anionzo_compact_prompt");
    }
  }
  ...
  if (hooksNeedMerge) {
    actions.push("install_anionzo_hook_entries");
  }

  if (hookScriptsNeedUpdate(repoRoot)) {
    actions.push("sync_anionzo_hook_scripts");
  }

  if (supportScriptsNeedUpdate(repoRoot)) {
    actions.push("sync_anionzo_support_scripts");
  }

  if (needsStateUpdate(repoRoot)) {
    actions.push("write_.anionzo/state.json");
  }

  if (onboarding.plugin_version !== pluginVersion) {
    actions.push("write_.anionzo/onboarding.json");
  }
...
export function applyRepo(repoRoot, allowCompactPromptReplace) {
  ...
  const statePayload = fs.existsSync(statePath)
    ? normalizeAnionzoState(readAnionzoState(repoRoot))
    : buildDefaultState();
  writeAnionzoState(repoRoot, statePayload);
  ...
  if (configResult.changes.includes("compact_prompt_conflict_preserved")) {
    ...
    onboardingNotes.push(
      "Existing compact_prompt preserved; Anionzo compaction recovery was not installed.",
    );
  }
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--help" || arg === "-h") {
      process.stdout.write(
        [
          "Usage: onboard_anionzo.mjs [--repo-root <path>] [--apply] [--allow-compact-prompt-replace]",
          "",
          "Checks or applies Anionzo repo onboarding.",
        ].join("\n"),
      );
      process.exit(0);
    }
