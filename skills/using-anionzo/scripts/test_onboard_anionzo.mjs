...
import { applyRepo, checkRepo, getNodeRuntimeStatus } from "./onboard_anionzo.mjs";
import { buildAnionzoDependencyReport } from "./anionzo_dependencies.mjs";

const LOCAL_ONBOARD_SCRIPT_PATH = fileURLToPath(new URL("./onboard_anionzo.mjs", import.meta.url));
const LOCAL_USING_ANIONZO_SKILL_PATH = fileURLToPath(new URL("../SKILL.md", import.meta.url));
const LOCAL_REPO_ROOT = fileURLToPath(new URL("../../../../../", import.meta.url));

function runSessionStartHook(root, payload = { cwd: root }, env = {}) {
  const hookPath = path.join(root, ".codex", "hooks", "anionzo_session_start.mjs");
...
test("applyRepo creates full repo onboarding with node-based hooks", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "anionzo-onboard-"));
  ...
  try {
    ...
    assert.match(fs.readFileSync(path.join(root, "AGENTS.md"), "utf8"), /Anionzo Workflow/);
    assert.ok(fs.existsSync(path.join(root, ".codex", "config.toml")));
    assert.ok(fs.existsSync(path.join(root, ".codex", "hooks.json")));
    assert.ok(fs.existsSync(path.join(root, ".anionzo", "onboarding.json")));
    assert.ok(fs.existsSync(path.join(root, ".anionzo", "state.json")));
    assert.ok(fs.existsSync(path.join(root, ".codex", "hooks", "anionzo_session_start.mjs")));
    assert.ok(fs.existsSync(path.join(root, ".codex", "anionzo_status.mjs")));
    assert.ok(fs.existsSync(path.join(root, ".codex", "anionzo_state.mjs")));
    assert.ok(fs.existsSync(path.join(root, ".codex", "anionzo_dependencies.mjs")));
    assert.match(
      fs.readFileSync(path.join(root, ".codex", "hooks.json"), "utf8"),
      /node \.codex\/hooks\/anionzo_session_start\.mjs/,
    );
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(root, ".anionzo", "state.json"), "utf8")).phase,
      "idle",
    );
  } finally {
...
test("applyRepo appends managed block to existing agents instructions", () => {
  ...
  try {
    ...
    assert.match(agentsText, /<!-- ANIONZO:START -->/);
    assert.equal((agentsText.match(/<!-- ANIONZO:START -->/g) || []).length, 1);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test("checkRepo flags stale python hook commands and legacy hook files", () => {
  ...
  try {
                ...
                hooks: [
                  {
                    type: "command",
                    command:
                      'python3 "$(git rev-parse --show-toplevel 2>/dev/null || pwd)/.codex/hooks/anionzo_session_start.py"',
                    statusMessage: "Anionzo: session bootstrap",
                  },
                ],
              },
            ],
    ...
    fs.writeFileSync(path.join(hooksDir, "anionzo_session_start.py"), "# legacy\n", "utf8");

    const result = checkRepo(root);

    assert.equal(result.status, "needs_onboarding");
    assert.ok(result.actions.includes("install_anionzo_hook_entries"));
    assert.ok(result.actions.includes("sync_anionzo_hook_scripts"));
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});

test("installed anionzo_status script reports onboarding and state", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "anionzo-onboard-"));

  try {
    applyRepo(root, false);

    const stdout = execFileSync("node", [path.join(root, ".codex", "anionzo_status.mjs"), "--json"], {
      cwd: root,
      encoding: "utf8",
    });
...
test("installed anionzo_status reports gkg readiness for a supported indexed repo", async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "anionzo-gkg-"));
  ...
  try {
    ...
    const stdout = execFileSync("node", [path.join(root, ".codex", "anionzo_status.mjs"), "--json"], {
      cwd: root,
      encoding: "utf8",
      env: { ...process.env, ANIONZO_GKG_SERVER_URL: mockServer.url },
    });
    const status = JSON.parse(stdout);
...
test("installed anionzo_status text distinguishes missing commands from missing MCP config", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "anionzo-onboard-"));

  try {
    applyRepo(root, false);
    const skillsRoot = path.join(root, "plugins", "anionzo", "skills");
    ...
    const stdout = execFileSync("node", [path.join(root, ".codex", "anionzo_status.mjs")], {
      cwd: root,
      encoding: "utf8",
    });
    ...
    assert.match(stdout, /Affects: anionzo:alpha/);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test("dependency report distinguishes dependency-free packaged skills from uncovered ones", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "anionzo-coverage-"));
  const skillsRoot = path.join(root, "plugins", "anionzo", "skills");
  ...
  try {
    ...
    const report = buildAnionzoDependencyReport({
      repoRoot: root,
      skillsRoot,
    ...
    assert.deepEqual(report.uncovered_skills.map((skill) => skill.skill_name), ["anionzo:beta"]);

    const alpha = report.skills.find((skill) => skill.skill_name === "anionzo:alpha");
    const beta = report.skills.find((skill) => skill.skill_name === "anionzo:beta");
...
test("anionzo_status text surfaces uncovered packaged skills instead of skipping them", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "anionzo-coverage-"));
  ...
  try {
    ...
    assert.match(stdout, /anionzo:beta/);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test("checkRepo promotes missing dependency data into an operator-facing warning summary", () => {
  ...
  try {
    ...
    assert.deepEqual(warning.affected_skills, ["anionzo:alpha"]);
    assert.match(warning.message, /Dependency warning:/);
    assert.match(warning.message, /anionzo:alpha/);
...
test("session-start hook emits dependency warning with command-vs-MCP split when dependencies are missing", () => {
  ...
  try {
    ...
    assert.match(context, /anionzo:alpha/);
...
test("session-start hook surfaces gkg readiness guidance for supported repos that are not ready", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "anionzo-gkg-hook-"));
  ...
  try {
    ...
    const payload = runSessionStartHook(
      root,
      { cwd: root },
      { ANIONZO_GKG_SERVER_URL: "http://127.0.0.1:9" },
    );
    const context = payload.hookSpecificOutput.additionalContext;
...
test("entry surfaces share the same missing-command vs missing-MCP wording boundary", () => {
  ...
  try {
    ...
    const scoutText = execFileSync("node", [path.join(root, ".codex", "anionzo_status.mjs")], {
      cwd: root,
      encoding: "utf8",
    });
    ...
    assert.match(scoutText, /Affects: anionzo:alpha/);
    ...
    assert.match(onboardingWarning.message, /Affected skills: anionzo:alpha/);
    ...
    assert.match(startupContext, /Affected skills: anionzo:alpha/);

    const skillText = fs.readFileSync(LOCAL_USING_ANIONZO_SKILL_PATH, "utf8");
...
test("dependency helper marks missing command and missing mcp_server dependencies", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "anionzo-deps-"));
  const skillsRoot = path.join(root, "plugins", "anionzo", "skills");
...
test("dependency helper respects declared MCP config_sources and can use packaged manifests", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "anionzo-mcp-sources-"));
  const pluginRoot = path.join(root, "plugins", "anionzo");
  const skillsRoot = path.join(root, "plugins", "anionzo", "skills");
...
test("dependency helper still accepts legacy root-level plugin MCP manifests", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "anionzo-mcp-legacy-"));
  const pluginRoot = path.join(root, "plugins", "anionzo");
  const skillsRoot = path.join(root, "plugins", "anionzo", "skills");
...
test("packaged Anionzo inventory stays fully covered and the docs explain the declaration contract", () => {
  const report = buildAnionzoDependencyReport({ repoRoot: LOCAL_REPO_ROOT });
  const skillText = fs.readFileSync(LOCAL_USING_ANIONZO_SKILL_PATH, "utf8");
  const pluginManifest = JSON.parse(
    fs.readFileSync(
      path.join(LOCAL_REPO_ROOT, "plugins", "anionzo", ".codex-plugin", "plugin.json"),
      "utf8",
    ),
  );
  const pluginMcp = JSON.parse(
    fs.readFileSync(path.join(LOCAL_REPO_ROOT, "plugins", "anionzo", ".mcp.json"), "utf8"),
  );
