...
function readGkgServerStatus(repoRoot) {
  const serverUrl = process.env.ANIONZO_GKG_SERVER_URL || GKG_SERVER_URL;
  ...
  try {
    ...
    while (true) {
      if (
        ...
        fs.existsSync(path.join(candidate, ".anionzo", "onboarding.json"))
      ) {
        return candidate;
      }
...
export function normalizeAnionzoState(state) {
...
export function getAnionzoStatePaths(repoRoot) {
  return {
    onboarding: path.join(repoRoot, ".anionzo", "onboarding.json"),
    stateJson: path.join(repoRoot, ".anionzo", "state.json"),
    stateMarkdown: path.join(repoRoot, ".anionzo", "STATE.md"),
    handoff: path.join(repoRoot, ".anionzo", "HANDOFF.json"),
    config: path.join(repoRoot, ".anionzo", "config.json"),
    agents: path.join(repoRoot, "AGENTS.md"),
    criticalPatterns: path.join(repoRoot, "history", "learnings", "critical-patterns.md"),
  };
}

export function readAnionzoState(repoRoot) {
  const paths = getAnionzoStatePaths(repoRoot);
  return normalizeAnionzoState(readJsonIfExists(paths.stateJson));
}

export function writeAnionzoState(repoRoot, nextState) {
  const paths = getAnionzoStatePaths(repoRoot);
  const normalized = normalizeAnionzoState(nextState);
...
function buildNextReads(status) {
  ...
  if (status.handoff.exists) {
    reads.push(".anionzo/HANDOFF.json");
  }

  if (status.state_json.exists) {
    reads.push(".anionzo/state.json");
  }

  if (status.state_markdown.exists) {
    reads.push(".anionzo/STATE.md");
  }
...
function buildRecommendedActions(status) {
  if (!status.onboarding.exists) {
    return [
      "Run Anionzo onboarding before continuing.",
      "Use the plugin onboarding script in plugins/anionzo/skills/using-anionzo/scripts/.",
    ];
  }
  ...
  return [
    "Use this status snapshot to choose the next Anionzo skill.",
    "If you move into planning or execution, read critical-patterns.md first when it exists.",
  ];
}

export function readAnionzoStatus(repoRoot) {
  const paths = getAnionzoStatePaths(repoRoot);
  ...
  const status = {
    ...
    state_json: {
      ...
      ...normalizeAnionzoState(stateJson),
    },
    state_markdown: {
...
export function renderAnionzoStatus(status) {
