...
import { readGkgReadiness } from "../anionzo_state.mjs";

function findRepoRoot(start) {
  let candidate = path.resolve(start || ".");
  while (true) {
    if (fs.existsSync(path.join(candidate, ".anionzo", "onboarding.json"))) {
      return candidate;
    }
...
function buildSessionDependencyWarning(repoRoot) {
  ...
  try {
    dependencyHealth = buildAnionzoDependencyReport({ repoRoot });
  } catch {
    return "";
  }
  ...
  return (
    ...
    `so some Anionzo skills are degraded or unavailable. ` +
...
export async function main() {
  ...
  if (fs.existsSync(onboardingPath)) {
    notes.push(
      "Anionzo onboarding is installed for this repo. Read AGENTS.md, then run node .codex/anionzo_status.mjs --json for a quick scout before substantive work.",
    );
  } else {
    notes.push("Anionzo onboarding is missing in this repo. Load anionzo:using-anionzo before continuing.");
  }
