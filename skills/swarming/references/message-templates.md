...
All workers: join this thread, post startup acknowledgment, then load the anionzo:executing skill.
...
AGENTS.md: read
Status: Loading anionzo:executing skill.
...
Next step: Invoke the anionzo:reviewing skill.
```
...
Write to `.anionzo/HANDOFF.json` when the swarm coordinator context exceeds 65%:

```json
{
  "schema_version": "1.0",
  "format": "anionzo-swarm-handoff",
  "session": {
    "id": "anionzo-swarm-<YYYYMMDD-HHMMSS>",
  ...
  "resume_instructions": {
    ...
    "read_first": [".anionzo/STATE.md", ".anionzo/HANDOFF.json"],
    "check_mail": true,
